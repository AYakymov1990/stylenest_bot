...
from sqlalchemy import BigInteger, String, Integer, DateTime, Enum, Text, JSON, Index
...
class Subscription(Base):
    ...
    # МЕТКИ-ОДИНОЖДЫ для рассылок (точно колонки БД, не property!)
    reminded_3d_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True
    )
    reminded_1d_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True
    )
    reminded_expired_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True
    )

    __table_args__ = (
        Index("ix_subscriptions_tg_id_status", "tg_id", "status"),
